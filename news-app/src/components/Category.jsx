...
const Category = () => {
  ...
  const catKey = useMemo(() => (categoryParam || '').toLowerCase().trim(), [categoryParam]);

  const categoryPosts = useNewsStore((s) => s.categoryPosts);
  const loading = useNewsStore((s) => s.loading);
  const error = useNewsStore((s) => s.error);

  const fetchRef = useRef(useNewsStore.getState().fetchByCategory);

  useEffect(() => {
    if (!catKey) return;
    if ((categoryPosts?.[catKey] || []).length > 0) return;
    fetchRef.current(catKey);
  }, [catKey, categoryPosts]);

  const posts = categoryPosts?.[catKey] || [];
    ...
    <div className={styles.CategoryBg}>
      <div className="container">
        <div className={styles.breadcrumb}>
          ...
          <Icon icon="material-symbols-light:keyboard-arrow-right" className={styles.breadcrumbIcon} />
          <span className={styles.active}>{pretty(catKey) || 'Category'}</span>
        </div>
