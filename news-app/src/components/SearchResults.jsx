...
const SearchResults = () => {
  ...
  const { newsBySearch, searchNews, loading, error } = useNewsStore();

  const query = useMemo(() => {
    const fromParam = (qParam ?? '').trim();
    if (fromParam) return fromParam;
    const sp = new URLSearchParams(location.search);
    return (sp.get('q') ?? '').trim();
  }, [qParam, location.search]);

  useEffect(() => {
    if (!query) return;
    searchNews(query, {
      mode: 'q',       // or 'qInTitle' / 'qInMeta'
      language: 'en',
      country: 'us',
      maxPages: 2
    });
      ...
      <Link className={styles.postCard} to={`/posts/${encodeURIComponent(article.id || article.link || index)}`}>
        ...
        <div className={styles.pauthorBg}>
          <div className={styles.pauthor}>
            <img
              src={`https://ui-avatars.com/api/?name=${encodeURIComponent(article.author || 'Unknown')}&background=random`}
              alt="Author"
  ...
  const renderListRow = (article, index) => (
    <div className="col-12" key={article.id || article.link || index}>
      <Link
        to={`/posts/${encodeURIComponent(article.id || article.link || index)}`}
        className={styles.postCard}
        style={{ display: 'flex', gap: '16px', padding: '16px', alignItems: 'stretch', textDecoration: 'none' }}
      >
        <div style={{ flex: '0 0 220px', maxWidth: '220px', background: '#f6f6f6', borderRadius: '12px', overflow: 'hidden' }}>
          {article.image_url ? (
            <img
              src={article.image_url}
              alt={article.title || 'Article'}
        ...
        <div style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <h3 className={styles.postTitle} style={{ marginBottom: 8 }}>{article.title}</h3>
          <p className={styles.pContent} style={{ marginBottom: 12 }}>{article.description || 'No description available.'}</p>
          <div className={styles.pauthorBg} style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div className={styles.pauthor}>
              <img
                src={`https://ui-avatars.com/api/?name=${encodeURIComponent(article.author || 'Unknown')}&background=random`}
                alt="Author"
    ...
    <div className={styles.CategoryBg}>
      <div className="container">
        <div className={styles.breadcrumb}>
          ...
          <span className={styles.breadcrumbItem}>Search Results</span>
        </div>
        ...
        <div className={styles.filter}>
          ...
          <div className={styles.sort}>
            <Icon icon="pepicons-print:list" className={styles.sortIcon} style={{ opacity: view === 'list' ? 1 : 0.5, cursor: 'pointer' }} onClick={() => setView('list')} />
            <Icon icon="pepicons-print:grid" className={styles.sortIcon} style={{ opacity: view === 'grid' ? 1 : 0.5, cursor: 'pointer' }} onClick={() => setView('grid')} />
          </div>
        </div>
